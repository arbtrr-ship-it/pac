...
```
 ...
 │ User             │  │ Product                            │
 │ • id (PK)        │  │ • id (PK)                          │
 │ • workspace_id   │  │ • workspace_id (FK, UNIQUE)        │
 └──────────────────┘  │ • current_strategy_version_id (FK) │──┐
                       └─────────────────┬──────────────────┘  │ 1:1
                                         │ 1:N                 │ (активная
                                         ▼                     │  версия)
                       ┌────────────────────────────┐ ◄────────┘
 ...
 │ • id (PK)                  │   │ • id (PK)                    │
 │ • strategy_version_id (FK) │   │ • workspace_id (FK)          │
 │ • workspace_id (FK)        │   │ • strategy_version_id (FK)   │
 └────────────────────────────┘   │ • created_by (FK → User)     │
                                  └───────────────┬──────────────┘
                                                  │ 1:1
                                                  ▼
